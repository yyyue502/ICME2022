...
def factorize_weight(generator, layer_idx='all'):
    ...
    for idx in layers:
        ...
        if gan_type == 'pggan':
            ...
            # weight = np.fft.fft2(weight.cpu().detach().numpy(), axes=[0, 1]).reshape(512, -1).real
            # print(f'ori shape: {weight.shape}')
            # weight = weight.flip(2, 3).permute(1, 0, 2, 3).flatten(1)
            # print(f'aft shape: {weight.shape}')
        elif gan_type in ['stylegan', 'stylegan2']:
            weight = generator.synthesis.__getattr__(layer_name).style.weight.T
        if idx > 0:
            filter = generator.synthesis.__getattr__(layer_name).weight.T
            transform_coeff = np.fft.fft2(filter.cpu().detach().numpy(), axes=[0, 1])
            print(filter.shape, weight.shape, transform_coeff.shape)
        # weights.append(weight.cpu().detach().numpy())
        # weights.append(weight)
        if transform_coeff is not None:
            weights.append(transform_coeff.reshape(512, -1).real)
